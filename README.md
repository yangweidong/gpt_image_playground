...
- **透明 PNG 后处理**：普通生图模式下可开启透明 PNG，应用会请求纯色背景 PNG，并在本地去除背景色后保存为透明 PNG。
...
在普通生图模式中，参数栏可开启 **透明PNG**。开启后：

- 请求会强制使用 `PNG` 输出，并关闭压缩率参数。
- 应用会在提示词后追加透明 PNG 工作流说明，要求模型使用纯绿色或纯洋红色背景。
- 图片返回后会在浏览器本地执行背景色去除，并把结果作为透明 PNG 保存到本地历史记录。

此功能是面向图标、贴纸、单主体素材的本地后处理流程，并非 API 原生透明通道。若主体边缘含有复杂发丝、半透明材质、强反光或与键色接近的颜色，仍可能出现边缘残留或误抠除。当前该开关仅在普通生图模式的 OpenAI 兼容接口中启用；Agent 模式、fal.ai 与自定义 JSON 服务商暂不启用。
